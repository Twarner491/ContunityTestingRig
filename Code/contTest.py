...
def getHeaderFormat():
    with open("headerformat.txt", "r") as file: #basically a try catch loop
        ...
        global colors
        colors = split[1][1:(len(split[1])-1)].split(',') #colors list is set to the colors defined in the format file
        if(int(len(colors))!= int(split[0])): #check if the format is correct, else ask to reformat
            print("Missing pin colors, add 'EMPTY' for a disconnected pin in the header")
            return -1 #incorrect header format error case
        global numPins
        numPins = split[0] #numPins is the number of output pins necessary
        return 0
